...
/// A cell filled by cycling the given palette indices, as linear pixels.
fn cell_of(palette: &[Rgb], indices: &[u8], len: usize) -> Vec<[f32; 3]> {
    (0..len)
        .map(|i| {
            let c = palette[usize::from(indices[i % indices.len()])];
            srgb8_to_linear([c.r, c.g, c.b])
        })
        .collect()
}
...
fn c64_multi_background_slot_is_honoured() {
    ...
    // and red (2).
    let cell = cell_of(pal, &[6, 1, 2, 6], 32);

    let (choice, score) = searcher.c64_multi(&cell, 6);
...
fn c64_multi_cell_without_the_background_colour_still_renders() {
    ...
    let cell = cell_of(pal, &[3, 4, 5, 3], 32);

    let (choice, score) = searcher.c64_multi(&cell, 0);
...
fn background_histogram_picks_most_frequent_colour() {
    let pal_rgb = palette_of("commodore-c64");
    let pal = PaletteData::new(pal_rgb, Metric::OkLab);
    // 6 blue pixels, 2 white.
    let pixels: Vec<[f32; 3]> = [6u8, 6, 6, 1, 6, 6, 1, 6]
        .iter()
        .map(|&i| {
            let c = pal_rgb[usize::from(i)];
            srgb8_to_linear([c.r, c.g, c.b])
        })
        .collect();
    let img = LinearImage {
    ...
    assert_eq!(choose_background(&img, &pal), 6);
}
